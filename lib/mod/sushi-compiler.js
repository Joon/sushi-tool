(function() {
  var Multispinner, NodePDF, SushiTemplateLoader, async, bodyParser, factor, fromDir, fs, glob, livereload, nodePDFproperties, open, os, pHeight, pWidth, path, port, reCopyFiles, toDir, watch, webPrefix, webpages;
  ...
  reCopyFiles = function(param1, param2) {
    ...
    return fs.copySync(working_dir, target_file);
  };
        ...
        function(callback) {
          ...
          return app.listen(port, function() {
            ...
            if (loadLiveReload) {
              ...
              server.watch(process.platform === 'win32' ? target_file : working_dir);
              console.log("Watching folder at " + (process.platform === 'win32' ? target_file : working_dir));
              if (process.platform === 'win32') {
                fromDir = working_dir;
                toDir = target_file;
                watch(working_dir, {
